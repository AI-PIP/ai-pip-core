# @ai-pip/core — Full README

This is the **complete** README with all examples, use cases, and detailed documentation. For a shorter overview (e.g. on npm), see the [root README](../README.md). Links below to CHANGELOG, FEATURE, and LICENSE use `../` so they work when viewing this file from the `docs/` folder.

---

# @ai-pip/core

> Core implementation of the AI-PIP protocol. Provides layered, zero-trust context processing (CSL, ISL, AAL) and transversal CPE to ensure the integrity of each layer for greater security.
...
- **Shared**: Lineage, audit formatters, and CPE (envelope) as transversal. Pure functions shared across the core.
...
The **processing pipeline** is: **CSL → ISL** (and optionally **AAL** consumes the ISL signal). **CPE** does not sit in that sequence: it is a **transversal** capability that **ensures the integrity of each layer** for greater security. You run the pipeline, then **optionally** wrap the result with `envelope(islResult, secretKey)` to obtain a signed, tamper-evident payload; the envelope allows verification that the output of each layer has not been altered. This provides a security guarantee across the whole chain and keeps CPE reusable regardless of pipeline shape (e.g. with or without AAL).
...
- Policy enforcement
- Remediation execution (e.g. AI-powered cleanup using the core’s remediation plan)
...
import { createAnomalyScore, resolveAgentAction, resolveAgentActionWithScore, buildRemediationPlan } from '@ai-pip/core/aal'
import type { AnomalyScore, AgentPolicy, RemediationPlan } from '@ai-pip/core/aal'

// Import from CPE (transversal; lives in shared/envelope, exported as @ai-pip/core/cpe)
...
const auditReport = {
  ...
  layers: {
    ...
    isl: {
      ...
      segmentsWithDetections: islResult.segments.filter(s => s.piDetection?.detections?.length).length
    },
    cpe: {
...
import type { ISLSignal, AgentPolicy, RemediationPlan } from '@ai-pip/core'
...
// 7. Build remediation plan (what to clean, not how). SDK/AI performs the actual cleanup.
const remediationPlan: RemediationPlan = buildRemediationPlan(islResult, policy)
if (remediationPlan.needsRemediation) {
  console.log('Strategy:', remediationPlan.strategy)           // e.g. 'AI_CLEANUP'
  console.log('Target segments:', remediationPlan.targetSegments) // segment IDs to clean
  console.log('Goals:', remediationPlan.goals)                 // e.g. ['remove_prompt_injection']
  console.log('Constraints:', remediationPlan.constraints)      // e.g. ['preserve_user_intent']
  // Use islResult + remediationPlan in the SDK to run your AI cleanup tool on targetSegments.
}
```
...
3. **Separation of Concerns**: 
   ...
   - AAL: Evaluates signals, applies policies, builds remediation plans (what to clean; target segments, goals, constraints)
   - SDK: Executes actions, performs remediation (e.g. AI cleanup using the plan), manages state
...
console.log(formatAALForAudit(reason, remediationPlan))
...
const fullReport = formatPipelineAuditFull(cslResult, islResult, islSignal, reason, remediationPlan, cpeResult, {
  runId,
  generatedAt,
  includeCpe: true,
...
const payload = buildFullAuditPayload(cslResult, islResult, islSignal, reason, { runId, generatedAt, remediationPlan, cpe: cpeResult })
const jsonReport = formatPipelineAuditAsJson(cslResult, islResult, islSignal, reason, { runId, generatedAt, remediationPlan, compact: true })
...
- **`formatAALForAudit(reason, remediationPlan?)`**: Formats AAL decision reason and optional remediation plan.
...
- **`formatPipelineAuditFull(csl, isl, signal, aalReason, remediationPlan?, cpe?, options?)`**: Full pipeline report (CSL → ISL → Signal → AAL → optional CPE) with **run id** and **generated at** timestamp; lineage in each section.
...
- **JSON variant**: **`buildFullAuditPayload(csl, isl, signal, reason, options?)`** returns a JSON-serializable object (runId, generatedAt, summary, sections with lineage). Pass **`options.remediationPlan`** and **`options.cpe`** when needed. **`formatPipelineAuditAsJson(...)`** returns the JSON string; use **`options.compact: true`** for one-line output (logs, SIEM).
...
| **Policy-based moderation** | CSL → ISL → emitSignal → AAL | Get risk signal from ISL, resolve ALLOW/WARN/BLOCK; use `buildRemediationPlan(islResult, policy)` to get target segments, goals, and constraints; SDK (e.g. AI cleanup) performs remediation. |
...
const remediationPlan = buildRemediationPlan(islResult, policy)
// Use action, reason, remediationPlan; when needsRemediation, SDK runs cleanup on targetSegments (goals/constraints).
```
...
const runId = createAuditRunId()
const fullReport = formatPipelineAuditFull(cslResult, islResult, signal, reason, remediationPlan, cpeResult, {
  runId, generatedAt: Date.now(), includeCpe: true
})
logger.info(fullReport)

// JSON for SIEM / machine consumption
const jsonReport = formatPipelineAuditAsJson(cslResult, islResult, signal, reason, { runId, generatedAt: Date.now(), remediationPlan, compact: true })
logger.info(jsonReport)
